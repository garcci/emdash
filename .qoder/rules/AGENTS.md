...
| 2025-06-09 | 【第三批-架构】提取重复 fadeInUp keyframes 到全局样式；修复 ArticleHero/RelatedPosts/posts 的类型安全问题 | `src/layouts/Base.astro`, `src/components/ArticleHeader.astro`, `src/components/PageHeader.astro`, `src/components/ArticleHero.astro`, `src/components/RelatedPosts.astro`, `src/pages/posts/[slug].astro` |
| 2025-06-09 | 【第二批-视觉】暗色模式 accent 对比度提升；主题切换平滑过渡；focus-visible 限定交互元素；prefers-reduced-motion 完善 | `src/layouts/Base.astro`, `src/styles/theme.css`, `src/components/EmptyState.astro`, `src/components/SearchForm.astro`, `src/components/PostCard.astro` |
| 2025-06-09 | 【第一批-性能】移除 Paper Texture SVG filter；阅读进度条 aria 节流；Glassmorphism backdrop-filter 降级；分页限制；will-change 动态化 | `src/layouts/Base.astro`, `src/components/SiteHeader.astro`, `src/components/SiteFooter.astro`, `src/pages/posts/index.astro`, `src/components/PostCard.astro`, `src/components/FeaturedPost.astro` |
| 2025-06-09 | 【修复】同步 theme.css 注释值（--glass-blur/--shadow-image-dark）；SiteFooter theme-switcher 补充 prefers-reduced-motion；Base.astro html transition 添加媒体查询保护 | `src/styles/theme.css`, `src/components/SiteFooter.astro`, `src/layouts/Base.astro` |
